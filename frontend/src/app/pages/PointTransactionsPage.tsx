...
const pointFilters: Array<{ value: PointFilter; label: string }> = [
  ...
  { value: 'CHARGE', label: pointTypeLabels.CHARGE },
  { value: 'CHARGE_CANCELLED', label: pointTypeLabels.CHARGE_CANCELLED },
  ...
  { value: 'REPORT_REWARD', label: pointTypeLabels.REPORT_REWARD },
  { value: 'REVIEW_REWARD', label: pointTypeLabels.REVIEW_REWARD },
];
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        setError('포인트 거래 내역을 불러오지 못했습니다.');
      } finally {
        setLoading(false);
      }
    };
...
function PointTransactionItem({ transaction }: { transaction: PointTransactionResponse }) {
  ...
  const typeLabel = pointTypeLabels[transaction.transactionType] || transaction.transactionType;
