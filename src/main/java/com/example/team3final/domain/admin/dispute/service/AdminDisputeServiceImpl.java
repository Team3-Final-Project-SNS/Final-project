...
public class AdminDisputeServiceImpl implements AdminDisputeService {
    ...
    public GetAdminDisputeResponseDto getDispute(Long adminId, Long disputeId) {
        ...
        // 이의 제기 조회
        Dispute dispute = disputeInternalService.getDisputeById(disputeId);
        ...
        // 제출자 닉네임 조회
        String applicantNickname = userInternalService.getUserInfo(dispute.getSubmitterId()).nickname();

        // matchId는 postId로 조회
        Long postId = matchInternalService.getMatchInfo(dispute.getMatchId()).postId();

        // 만남인증 정보 조회 (GPS 인증 시각 포함)
        MeetVerification meetVerification = meetVerificationInternalService.getByMatchId(dispute.getMatchId());

        // 채팅 내역 조회 — chatRoomId 없으면 빈 리스트
        Long chatRoomId = chatInternalService.getChatRoomIdByPostId(postId);
        List<ChatMessageResponseDto> messages =
                chatRoomId != null ? chatInternalService.getChatMessagesForAdmin(chatRoomId) : List.of();
    ...
    public PageResponseDto<GetAdminDisputesResponseDto> getDisputes(Long adminId, DisputeStatus status, Pageable pageable) {
        ...
        Page<Dispute> disputes = disputeInternalService.getDisputesForAdmin(status, pageable);
        ...
        Map<Long, String> nicknameMap = userInternalService.getUserNicknameMap(submitterIds);
    ...
    public AdminJudgeDisputeResponseDto judgeDispute(Long adminId, Long disputeId, AdminJudgeDisputeRequestDto requestDto) {
        ...
        int refundedPoint = switch (requestDto.getStatus()) {
            ...
            case ACCEPTED -> {
                ...
                yield matchNoShowService.completeSingleMatchByDispute(
                        dispute.getMatchId(),
