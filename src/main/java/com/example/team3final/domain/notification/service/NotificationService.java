...
public interface NotificationService {
   ...
   // 전체 읽음 처리
    UpdateAllNotificationsReadResponseDto updateAllNotificationsRead(
            NotificationReceiverType receiverType, Long receiverId);

    // 단건 읽음 처리
    // userId: 요청자 ID (본인 알림만 처리 가능)
    // notificationId: 읽음 처리할 알림 ID
    UpdateNotificationReadResponseDto updateNotificationRead(
            NotificationReceiverType receiverType, Long receiverId, Long notificationId);

    // 미확인 알림 카운트
    GetUnreadCountResponseDto getUnreadCount(NotificationReceiverType receiverType, Long receiverId);

    // SSE 연결
    SseEmitter subscribe(NotificationReceiverType receiverType, Long receiverId);
}
